...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePhase2UI() {
        ...
        document.querySelectorAll('.employee-section').forEach(section => {
            ...
            // Preserve current availability state before removing grid
            const currentAvailability = {};
            const oldGrid = section.querySelector('.availability-grid');
            if (oldGrid) {
                oldGrid.querySelectorAll('.availability-checkbox').forEach(checkbox => {
                    const key = `${employeeName}_${checkbox.dataset.day}_${checkbox.dataset.shift}`;
                    currentAvailability[key] = checkbox.checked;
                });
                oldGrid.remove();
            }
            
            const newGrid = createAvailabilityGrid(employeeId, employeeName, currentAvailability);
    ...
    function createAvailabilityGrid(employeeId, employeeName, availabilityData = null) {
        ...
        days.forEach((day, index) => {
            ...
            shifts.forEach(shift => {
                ...
                if (availabilityData) {
                    ...
                    const key = `${employeeName}_${day}_${shift.name}`;
                    isChecked = availabilityData[key] || false;
                    // Debug: Log what we're checking
                    if (employeeId <= 2) { // Only log for first 2 employees to avoid spam
                        console.log(`Employee: ${employeeName}, Day: ${day}, Shift: ${shift.name}, Key: ${key}, Available: ${isChecked}`);
                    }
                }
                ...
                checkboxWrapper.innerHTML = `
                    <input class="form-check-input availability-checkbox" type="checkbox"
                           ...
                           data-day="${day}"
                           data-shift="${shift.name}" ${isChecked ? 'checked' : ''}>
    ...
    async function loadExampleData() {
        try {
            const response = await fetch('/api/generate_example_data');
            const data = await response.json();
            
            if (data.status === 'success') {
                const exampleData = data.data;
                
                // Debug: Log the first employee's availability data
                console.log('Example data received:', exampleData);
                if (exampleData.employees.length > 0) {
                    console.log('First employee availability:', exampleData.employees[0].availability);
                }
                
                // Clear existing employee sections
                employeeSectionsContainer.innerHTML = '';
                employeeCounter = 0;
                
                // Update days selection
                daysCheckboxes.forEach(checkbox => {
                    checkbox.checked = exampleData.days.includes(checkbox.value);
                });
                
                // Update shifts
                shiftsContainer.innerHTML = '';
                exampleData.shifts.forEach(shift => {
                    const div = document.createElement('div');
                    div.classList.add('shift-row');
                    div.innerHTML = `
                        <div class="shift-input-group">
                            <label>Shift Name</label>
                            <input type="text" class="form-control shift-name" value="${shift.name}" placeholder="e.g., Morning">
                        </div>
                        <div class="shift-input-group">
                            <label>Hours</label>
                            <input type="number" class="form-control shift-hours" value="${shift.hours}" min="1">
                        </div>
                        <button type="button" class="btn btn-remove remove-shift-btn" title="Remove shift">
                            <i class="bi bi-trash"></i>
                        </button>
                ...
                if (exampleData.full_time_hours_per_week) {
                    document.getElementById('fullTimeHours').value = exampleData.full_time_hours_per_week;
                }
                
                // Update responsible required
                responsibleRequiredOverallCheckbox.checked = exampleData.responsible_required_overall;
                
                // Create employee sections from example data
                exampleData.employees.forEach(emp => {
                    employeeCounter++;
                    const employeeId = employeeCounter;
                    ...
                    const isUnlimited = emp.min_hours === 0 && emp.max_hours === 0;
                    const isFulltime = emp.is_full_time;
                    ...
                    div.innerHTML = `
                        ...
                        <div class="min-max-hours-section">
                            ...
                            <div class="row g-3">
                                <div class="col-md-3">
                                    <label class="form-label">${t('min_hours')}</label>
                                    <input type="number" class="form-control min-hours-input" value="${emp.min_hours}" min="0" data-employee-id="${employeeId}" ${isUnlimited || isFulltime ? 'disabled' : ''}>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label">${t('max_hours')}</label>
                                    <input type="number" class="form-control max-hours-input" value="${emp.max_hours}" min="0" data-employee-id="${employeeId}" ${isUnlimited || isFulltime ? 'disabled' : ''}>
                                </div>
                                <div class="col-md-3">
                                    <div class="toggle-option">
                                        <input class="form-check-input unlimited-hours-checkbox" type="checkbox" id="unlimitedHours${employeeId}" data-employee-id="${employeeId}" ${isUnlimited ? 'checked' : ''} ${isFulltime ? 'disabled' : ''}>
                                        <label class="form-check-label" for="unlimitedHours${employeeId}">${t('unlimited')}</label>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="toggle-option">
                                        <input class="form-check-input fulltime-hours-checkbox" type="checkbox" id="fulltimeHours${employeeId}" data-employee-id="${employeeId}" ${isFulltime ? 'checked' : ''} ${isUnlimited ? 'disabled' : ''}>
                                        <label class="form-check-label" for="fulltimeHours${employeeId}">${t('fulltime')}</label>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="wage-section">
                            <label for="wageInput${employeeId}" class="form-label">${t('hourly_wage')} ($)</label>
                            <input type="number" class="form-control wage-input" id="wageInput${employeeId}" value="${emp.wage}" min="0" data-employee-id="${employeeId}" ${isFulltime ? 'disabled' : ''}>
                            <div class="toggle-option mt-2">
                                <input class="form-check-input same-wage-checkbox" type="checkbox" id="sameWage${employeeId}">
                                <label class="form-check-label" for="sameWage${employeeId}">${t('apply_to_all')}</label>
                            </div>
                    ...
                    const availabilityGrid = createAvailabilityGrid(employeeId, emp.name, emp.availability);
                    const minMaxHoursSection = div.querySelector('.min-max-hours-section');
                    minMaxHoursSection.parentNode.insertBefore(availabilityGrid, minMaxHoursSection);
                ...
                // Update min/max employees per shift with example data
                if (sameMinMaxForAllShiftsCheckbox.checked) {
                    // Clear the checkbox to enable individual settings
                    sameMinMaxForAllShiftsCheckbox.checked = false;
                    sameMinMaxPerShiftAllDaysCheckbox.checked = false;
                    toggleMinMaxInput();
                }
                
                // Set individual min/max values from example data
                setTimeout(() => {
                    populateMinMaxGrid();
                    Object.keys(exampleData.min_employees_per_shift).forEach(key => {
                        const [day, shift] = key.split('_');
                        const minInput = document.querySelector(`.min-emp-input[data-day="${day}"][data-shift="${shift}"]`);
                        const maxInput = document.querySelector(`.max-emp-input[data-day="${day}"][data-shift="${shift}"]`);
                        
                        if (minInput) minInput.value = exampleData.min_employees_per_shift[key];
                        if (maxInput) maxInput.value = exampleData.max_employees_per_shift[key];
                    });
                }, 100);
                ...
                alert('Example data loaded successfully! 20 employees with realistic scheduling data have been added.');
            }
        } catch (error) {
    ...
    // Add event listener for load example data button
